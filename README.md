...
Playwright covers desktop/mobile account journeys, recipient signup from a share link, note editing and revocation, combined movie filters, rating edits, watchlist removal, hidden recommendations, password changes, and real authenticator setup/login/removal. It also checks motion, keyboard access, reduced motion, WebGL fallback, renderer cleanup, and retry after a simulated collection outage. All successful feature requests use the live Docker database. Authenticator scenarios wait for fresh 30-second codes; their traces and automatic screenshots are disabled to keep setup keys out of artifacts.

Artifacts are saved under `frontend/test-results/`. HTTPS certificate validation is enabled in every browser context. Trust the exported development CA before running the suite; on Windows, `scripts/start.ps1 -TrustLocalCertificate` handles browser trust. Node-based test requests also need `NODE_EXTRA_CA_CERTS` set to the absolute path of `secrets/local-ca.crt` before starting Playwright. If an HTTPS inspection proxy is present, use a PEM file containing both the development CA and your already trusted proxy CA.
